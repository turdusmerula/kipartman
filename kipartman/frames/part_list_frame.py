...
class PartCategory(helper.tree2.TreeContainerItem):
...
class Part(helper.tree2.TreeContainerLazyItem):
...
class TreeManagerParts(helper.tree2.TreeManager):
...
class TreeModelPart(helper.tree2.TreeModel):
...
class PartListFrame(PanelPartList): 
    ...
    def onToggleCategoryPathClicked( self, event ):
        self.flat = self.toolbar_part.GetToolState(self.toggle_part_path.GetId())
