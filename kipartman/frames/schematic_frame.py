...
from kicad.kicad_file_manager import KicadFileManager, EVT_FILE_CHANGED
import wx
import re
import os
...
class SchematicFrame(PanelSchematic): 
    def __init__(self, parent, file):
        ...
        self.file_managers = []
        self.Bind( EVT_FILE_CHANGED, self.onSchematicChanged )
    ...
    def _load_schematic(self):
        ...
        # add files watchdogs
        self.file_managers.append(KicadFileManager(self, os.path.abspath(os.path.dirname(self.file))))
        for sheet in self.schematic.objects.sheets:
            self.file_managers.append(KicadFileManager(self, os.path.abspath(os.path.join(os.path.dirname(self.schematic.filename), os.path.dirname(sheet.file)))))
    ...
    def onSchematicChanged( self, event ):
        path_found = False
        for manager in self.file_managers:
            if manager.Path in event.path:
                path_found = True
...
#             self.data_frame.SetPart(part)
    ...
    def get_symbol(self, symbol):
        return os.path.basename(symbol.library.path).replace(".lib", "")+":"+symbol.name
     
    def get_footprint(self, footprint):
        lib = os.path.basename(footprint.library.path).replace(".pretty", "")
        footprint = os.path.basename(footprint.name).replace(".kicad_mod", "")
        return lib+":"+footprint
#     
    def associate_part(self, component, part):
        unit_symbol = None
        if part.symbol:
            unit_symbol = self.get_symbol(part.symbol)
             
        unit_footprint = None
        if part.footprint:
            unit_footprint = self.get_footprint(part.footprint)
         
        if unit_symbol:
            component.symbol = unit_symbol
        if unit_footprint:
            component.footprint = unit_footprint

        component.value = api.data.part.expanded_value(part, part.value)
    ...
    def onSelectPartCallback(self, part_event):
        ...
        KicadFileManager.DisableNotificationsForPath(os.path.dirname(self.schematic.filename), self.schematic.Save)
    ...
    def onToolExportBomClicked( self, event ):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                KicadFileManager.DisableNotificationsForPath(os.path.dirname(filename), bom.SaveFile, filename)
            except Exception as e:
                print_stack()
