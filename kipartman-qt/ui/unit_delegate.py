...
# @end
    ...
    def createEditor(self, parent, option, index):
        ...
        uic.loadUi('ui/unit_delegate.ui', self.widget)
        
        self.menuPrefix = QMenu(self.widget)
        for item in self.prefixes:
            if len(item)==1:
                self.menuPrefix.addSeparator()
            else:
                ...
                action = QAction(parent=self.menuPrefix, text=text)
                action.prefix = item
                action.triggered.connect(self.addPrefixAction)
                self.menuPrefix.addAction(action)
        ...
        for item in self.units:
            ...
            if isinstance(item, str):
                ...
                action = QAction(parent=self.menuUnit, text=text)
                action.unit = item
                action.triggered.connect(self.addUnitAction)
                submenu.addAction(action)
        
        self.widget.lineEdit.textChanged.connect(self.lineEditTextChanged)
        self.widget.toolButtonPrefix.clicked.connect(self.toolButtonPrefixTriggered)
        self.widget.toolButtonUnit.clicked.connect(self.toolButtonUnitTriggered)
    ...
    def toolButtonPrefixTriggered(self):
        ...
        self.menuPrefix.popup(self.widget.toolButtonPrefix.mapToGlobal(QPoint(rect.x(), rect.y()+rect.height())))

    def toolButtonUnitTriggered(self):
        rect = self.widget.toolButtonUnit.rect()
        self.menuUnit.popup(self.widget.toolButtonUnit.mapToGlobal(QPoint(rect.x(), rect.y()+rect.height())))

    def lineEditTextChanged(self):
        pass
            
    def addPrefixAction(self):
        item = self.sender().prefix
        if isinstance(item, tuple)==False:
            return
        prefix, description = item
        if isinstance(prefix, list):
            self.widget.lineEdit.insert(prefix[0])
        else:
            self.widget.lineEdit.insert(prefix)
        self.widget.lineEdit.setFocus()
    ...
    def addUnitAction(self):
        ...
        if len(item)==1:
            unit, = item
            self.widget.lineEdit.insert(unit)
        elif len(item)==2:
            unit, description = item
            if isinstance(unit, list):
                self.widget.lineEdit.insert(unit[0])
            else:
                self.widget.lineEdit.insert(unit)
        self.widget.lineEdit.setFocus()
