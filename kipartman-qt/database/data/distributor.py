...
def find(filters=[]):
    request = database.models.Distributor.objects
...
def create(**kwargs):
    return database.models.Distributor(**kwargs)

def save(distributor):
    distributor.save()

def delete(distributor):
    database.data.part_offer.find([database.data.part_offer.FilterDistributor(distributor)]).delete()
    distributor.delete()